					...
					<div className={cx({wrapinner: true,showclear: this.state.selectList.length !== 0})} onClick={this.handleWrapClick}>
						<div className={styles.checkwrap}>
						{
							this.state.selectList.map(({label,value})=>{
								return <span key={value} className={cx({
									checkItem: true,
									disabled: false,
						...
						{
							this.state.selectList.length === 0 ? (<span className={styles.placeholder}>{this.props.placeholder}</span>) : []
						}
						{
							this.state.selectList.length === 0 ? [] : (<span onClick={this.handleRemoveAll} ref={this.clearRef} className={styles.clear}>
						...
						}	
						{
							...
							(
								<span ref={this.checkRef} className={classnames(styles.icon,styles.checkall)} onClick={this.handleCheckAll}>
									<i className={classnames('iconfont','icon-quanxuan')}></i>
								</span>									
							) : (
								<span ref={this.downRef} className={classnames(styles.icon,styles.iconl)}>
